...
#End Region
    ...
    Private Sub Form_Splash_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        AgentInformation.AppVersion = "SorryFix-45"
        ...
        Try
            ...
            Select Case xScreen
                Case "Ready"
                    ...
                    End If
                    Try
                        Call dbClass.Activity_Get_SessionID("Idle")
                    Catch ex As Exception
                    ...
                    Try
                        dbClass.UpdateRealyTimeStatus(CallInformation.CallDirection, "")
                    Catch ex As Exception
                        Dim tmpText As String = "SetupScreen(UpdateRealyTimeStatus) nLine:" & xScreen & " " & "Exception:" & " " & ex.Message
                        Write_Log("", tmpText)
                    End Try
                ...
                Case "DND"
                    ...
                    Try
                        dbClass.Activity_Get_SessionID(AgentInformation.DNDCaption)
                    Catch ex As Exception
                        Dim tmpText As String = "SetupScreen(Activity_Get_SessionID) nLine:" & xScreen & " " & "Exception:" & " " & ex.Message
                        Write_Log("", tmpText)
                    End Try
                    Try
                        dbClass.UpdateRealyTimeStatus(OtherText, "")
                    Catch ex As Exception
                        Dim tmpText As String = "SetupScreen(UpdateRealyTimeStatus) nLine:" & xScreen & " " & "Exception:" & " " & ex.Message
                        Write_Log("", tmpText)
                    End Try
    ...
    Private Sub RunUploader()
        Try
            Write_Log("", "RunUploader(Run)")
            Shell(AgentInformation.AppPath & "\RecordingUploader.exe", AppWinStyle.Hide)
            Write_Log("", "RunUploader(Run)")
        Catch ex As Exception
    ...
    Private Sub ovsControl_Event_CallGeneric2(Sender As Object, e As TControl.Event_CallGeneric2_Args) Handles ovsControl.Event_CallGeneric2
        Write_Log("", "CallGeneric2(Start) nLine:" & e.eventName & ", sessionId=" & e.sessionId)
    ...
    Private Sub Process_CallConnected(Caller As String, Callee As String, sessionID As String, SIPString As String)
        Try
            ...
            Try
                dbClass.UpdateRealyTimeStatus(CallInformation.CallStatus, CallInformation.CalledID)
            Catch ex As Exception
                Dim tmpText As String = "CallConnected(UpdateRealyTimeStatus) nLine:" & sessionID & " " & "Exception:" & " " & ex.Message
                Write_Log("", tmpText)
            End Try
    ...
    Private Sub Process_CallRinging(ChannelNo As Long, LineNo As Long, Caller As String, Callee As String, sessionID As String, SIPString As String)
        Try
            ...
            Try
                ...
                dbClass.UpdateRealyTimeStatus(CallInformation.CallStatus, CallInformation.CallerID)
                updateStatusText("Update RT End (" & DateDiff(DateInterval.Second, time_Start, Now) & ")")

            Catch ex As Exception
                Dim tmpText As String = "CallRinging(UpdateRealyTimeStatus) nLine:" & sessionID & " " & "Exception:" & " " & ex.Message
                Write_Log("", tmpText)
            End Try
            ...
            If CallInformation.RefID_Server = -1 Then
                Try
                    ...
                    updateStatusText("Server_Get_SessionID End (" & DateDiff(DateInterval.Second, time_Start, Now) & ")")
                Catch ex As Exception
            ...
            If CallInformation.RefID_DB > -1 Then
                Try
                    ...
                    updateStatusText("SessionID_Get End (" & DateDiff(DateInterval.Second, time_Start, Now) & ")")
            ...
            Try
                ...
                Call dbClass.Activity_Get_SessionID("Ringing")
                updateStatusText("Activity_Get_SessionID End (" & DateDiff(DateInterval.Second, time_Start, Now) & ")")
            ...
            If CallInformation.CallDirection = "Inbound" Then
                ...
                Try
                    RingPlayer.Play()
                    Write_Log("APP", "Process_CallRinging(Ringing) Beep Played")
                Catch ex As Exception
            ...
            Write_Log("", "Process_CallRinging(End) nLine:" & CallInformation.nLine)

        Catch ex As Exception
            Write_Log("", "Process_CallRinging(Error) nLine:" & CallInformation.nLine & " " & ex.Message)
            'MessageBox.Show(Me, "Process_CallRinging(Error) nLine:" & CallInformation.nLine & " " & ex.Message)
        End Try
        ...
        Try
            ...
            If Button_Call_Hold.Tag = "" Then
                ...
                Try
                    dbClass.UpdateRealyTimeStatus(CallInformation.CallStatus, CallInformation.CalledID)
                Catch ex As Exception
                    Dim tmpText As String = "Call_Hold(UpdateRealyTimeStatus) nLine:" & CallInformation.CalledID & " " & "Exception:" & " " & ex.Message
                    Write_Log("", tmpText)
                End Try
                ...
                End If
                Try
                    Call dbClass.Activity_Get_SessionID("Hold")
                Catch ex As Exception
            ...
            Else
                ...
                Button_Call_Mute.Enabled = True
                Try
                    Call dbClass.Activity_Get_SessionID("UnHold")
                Catch ex As Exception
    ...
    Private Sub Timer_wsReader_Timer(msgFromWSClinet As String)
        ...
        Try
            ...
            If AgentInformation.LoggedIn = False Then
                ...
                End If
                If InStr(1, TheCommandText, "cmd{get-agents}") > 0 Then
                    GoTo processGetAgentsMsg
                End If
                If InStr(1, TheCommandText, "cmd{AgentStatus}") > 0 Then
                    GoTo processGetAgentsStats
                End If
                If InStr(1, TheCommandText, "cmd{ready-inbound}") > 0 Then
                    GoTo processGetAgentsStats
                End If
                If InStr(1, TheCommandText, "cmd{ready-outbound") > 0 Then
                    GoTo processGetAgentsStats
                End If
                Exit Sub
            End If
...
processGetAgentLoginMsg:
                    ...
                    ElseIf InStr(1, TheCommandText, "cmd{callagent,") > 0 Then
                        ...
                        'subCMD = Replace(subCMD, "}", "")

                        Dim subCMD() As String
                        Dim subCMD2 As String
                        'subCMD = Mid(TheCommandText, 15)
                        subCMD = Split(TheCommandText, ",")
                        subCMD2 = Replace(subCMD(1), "}", "")
    ...
    Private Sub IVR_TPINVerify(cmd As String)
        Try
            ...
            If Not _TerminalServerSession Then
                ovsControl.media_play(New ovsPSProPhony.TControl.media_play_Args(0, 0, CallInformation.nLineSessionID1, False, "\Sounds\" & sLanguage & "\TPINExisting.wav", True, "1234567890"))
            End If
            Application.DoEvents()
            Application.DoEvents()
            ...
            Else
                ...
                Application.DoEvents()
                If Not _TerminalServerSession Then
                    ovsControl.media_play(New ovsPSProPhony.TControl.media_play_Args(0, 0, CallInformation.nLineSessionID1, False, "\Sounds\" & sLanguage & "\TPINInvalid.wav", True, ""))
                End If
...
GetATMPIN1Again:
            ...
            If Len(sInputs) = 0 Then
                ...
                If Not _TerminalServerSession Then
                    ...
                    Application.DoEvents()
                    IVR_PlayFileAndGetDigits("\Sounds\" & sLanguage & "\ATMPINNoInput.wav", "No Input", 0, 0, "A")
                ...
                If iCurrentTryCount < iMaxInputTries Then
                    ...
                    If Not _TerminalServerSession Then
                        ...
                        IVR_PlayFileAndGetDigits("\Sounds\" & sLanguage & "\tryagain.wav", "Try Again", 0, 0, "A")
            ...
            If (Len(sInputs) <> 4) Or (IsNumeric(sInputs) = False) Then
                ...
                If Not _TerminalServerSession Then
                    ...
                    IVR_PlayFileAndGetDigits("\Sounds\" & sLanguage & "\ATMPINInvalidLength.wav", "Invalid Lenght", 0, 0, "A")
...
MaxTries:
                ...
                If Not _TerminalServerSession Then
                    ...
                    IVR_PlayFileAndGetDigits("\Sounds\" & sLanguage & "\ATMPINMaxTries.wav", "Max tries done", 0, 0, "A")
...
GetTPIN2Again:
            ...
            If Not _TerminalServerSession Then
                ...
                IVR_PlayFileAndGetDigits("\Sounds\" & sLanguage & "\ATMPINreEnter.wav", "ATM PIN 2", 21, 4, "*A")
            ...
            If (sInputs <> TPIN1) Then
                ...
                If iCurrentTryCount < iMaxInputTries Then
                    ...
                    If Not _TerminalServerSession Then
                        ...
                        IVR_PlayFileAndGetDigits("\Sounds\" & sLanguage & "\ATMPINNotSame.wav", "PIN not same", 0, 0, "A")
            ...
            If MakeATMPINResponse = True Then
                ...
                Application.DoEvents()
                If Not _TerminalServerSession Then
                    IVR_PlayFileAndGetDigits("\Sounds\" & sLanguage & "\ATMPINDone.wav", "ATM PIN Done", 0, 0, "A")
            ...
            Else
                ...
                Application.DoEvents()
                If Not _TerminalServerSession Then
                    IVR_PlayFileAndGetDigits("\Sounds\" & sLanguage & "\ATMPINFailed.wav", "ATM PIN Failed", 0, 0, "A")
...
GetTPIN1Again:
            ...
            Application.DoEvents()
            If Not _TerminalServerSession Then
                ovsControl.media_play(New ovsPSProPhony.TControl.media_play_Args(0, 0, CallInformation.nLineSessionID1, False, "\Sounds\" & sLanguage & "\TPINNNewEnter.wav", True, "1234567890"))
            End If
...
GetTPIN2Again:
            ...
            Application.DoEvents()
            If Not _TerminalServerSession Then
                ovsControl.media_play(New ovsPSProPhony.TControl.media_play_Args(0, 0, CallInformation.nLineSessionID1, False, "\Sounds\" & sLanguage & "\TPINreEnter.wav", True, "1234567890"))
            End If
            Application.DoEvents()
            tmpstring = ovsControl.media_getDigits(New TControl.media_getDigits_Args(0, 0, CallInformation.nLineSessionID1, False, (4 - Len(sInputs)), 15, True))
            _waitStartTime = DateTime.Now
            ...
            Else
                ...
                Application.DoEvents()
                If Not _TerminalServerSession Then
                    ovsControl.media_play(New ovsPSProPhony.TControl.media_play_Args(0, 0, CallInformation.nLineSessionID1, False, "\Sounds\" & sLanguage & "\TPINFailed.wav", True, ""))
                End If
...
sendAgain:
        ...
        Try
            ...
            If Trim(connectionID) <> "" Then
                ...
                Catch ex As Exception
                    Write_Log("", "sendTowsClinet:Sending YException(" & connectionCount & "):" & msgtoSend & ")")
                End Try
            Else
                For Each item In OnlineConnections
                    ...
                    Catch ex As Exception
                        Write_Log("", "sendTowsClinet:Sending LException(" & connectionCount & "):" & msgtoSend & ")")
                    End Try
                    Threading.Thread.Sleep(20)
                    Application.DoEvents()
                Next
            End If
            Write_Log("", "sendTowsClinet:sent(" & connectionCount & "):" & msgtoSend & ")")
        Catch ex As Exception
            Write_Log("", "sendTowsClinet:Error:" & ex.Message & " (" & msgtoSend & ")")
            'MessageBox.Show(Me, "sendTowsClinet:" & ex.Message & " (" & msgtoSend & ")")
        End Try
    End Sub
