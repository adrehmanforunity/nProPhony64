...
'' Aug-29-2024   /monitoring issue fix
'' Aug-29-2024   /install added
...
Public Class Form_Splash
    ...
    Private DisConnectedPlayer As SoundPlayer = New SoundPlayer()
...
#End Region
    ...
    Private Sub Form_Splash_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        For Each arg In args
            If arg.ToString.ToUpper.Trim() = "DTI=2" Then
                Configurations.bDisableTrayICON = True
                'Exit For
            End If
            If arg.ToString.ToUpper.Trim() = "/INSTALL" Then
                Dim identity As WindowsIdentity = WindowsIdentity.GetCurrent()
                Dim principal As New WindowsPrincipal(identity)
                If Not principal.IsInRole(WindowsBuiltInRole.Administrator) Then
                    MsgBox("To install nProPhony, please run the application with Administrative privileges.", MsgBoxStyle.Critical + MsgBoxStyle.OkOnly, "nProPhony Installer")
                Else
                    '' lets add required registory enteries
                    Dim exePath As String = Assembly.GetExecutingAssembly().Location
                    ' Base key for ProPhony protocol
                    Using key As RegistryKey = Registry.ClassesRoot.CreateSubKey("nprophony")
                        key.SetValue("", "URL:ProPhony Protocol")
                        key.SetValue("URL Protocol", "")
                    End Using
                    ' Shell key
                    Using shellKey As RegistryKey = Registry.ClassesRoot.CreateSubKey("nprophony\shell")
                    End Using
                    ' Open key
                    Using openKey As RegistryKey = Registry.ClassesRoot.CreateSubKey("nprophony\shell\open")
                    End Using
                    ' Command key with the dynamically obtained path to the executable
                    Using commandKey As RegistryKey = Registry.ClassesRoot.CreateSubKey("nprophony\shell\open\command")
                        commandKey.SetValue("", $"""{exePath}"" ""%1""")
                    End Using
                    MsgBox("nProPhony installed successfully", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "nProPhony Installer")
                End If
                End
                Exit For
            End If
        ...
        Try
            ...
            If System.IO.File.Exists(appPath & "\disconnected.wav") Then
                DisConnectedPlayer.SoundLocation = appPath & "\disconnected.wav"
                DisConnectedPlayer.LoadAsync()
            Else
                Write_Log("", " ********** No disconnected File found    ********** ")
            End If
    ...
    Public Function Process_DTMFOnConnected() As Boolean
        Try
            Process_DTMFOnConnected = False
            Write_Log("", "Process_DTMFOnConnected(Start) nLine:" & CallInformation.nLine)

            '' Inform the OVS PBX about call connection
            If GetINIString("Main", "DTMFOnConnected", "0") = "1" Then
                Write_Log("", "Process_DTMFOnConnected() Enabled To send DTMF")

                Try
                    Dim tmpDTMF As String = Trim(GetINIString("Main", "DTMFOnConnected_Digits", "*1"))
                    If tmpDTMF <> "" Then
                        Dim tmpDTMFDuration As Integer = 160
                        Dim tmpDTMFPlay As Boolean = True
                        Dim tmpDTMFMethod As Integer = 0
                        Try
                            tmpDTMFDuration = Val(Trim(GetINIString("Main", "DTMFOnConnected_Duration", "160")))
                        Catch ex As Exception
                            tmpDTMFDuration = 160
                        End Try
                        If Trim(GetINIString("Main", "DTMFOnConnected_Play", "1")) = "0" Then
                            tmpDTMFPlay = False
                        End If
                        If Trim(GetINIString("Main", "DTMFOnConnected_Method", "1")) = "1" Then
                            tmpDTMFMethod = False
                        End If
                        Write_Log("", $"Process_DTMFOnConnected(Loop) nLine: {CallInformation.nLine}, DTMFDuration: {tmpDTMFDuration}, DTMFPlay:{tmpDTMFPlay}, DTMFMethod:{tmpDTMFMethod}, Len:{Len(tmpDTMF)}")
                        For index = 0 To Len(tmpDTMF) - 1
                            Dim tmpDTMFToSend As Integer = -1
                            Select Case Mid(tmpDTMF, index + 1, 1)
                                Case 1, 2, 3, 4, 5, 6, 7, 8, 9, 0
                                    tmpDTMFToSend = Val(Mid(tmpDTMF, index + 1, 1))
                                Case "*"
                                    tmpDTMFToSend = 10
                                Case "#"
                                    tmpDTMFToSend = 11
                            ...
                            End Select
                            Try
                                If tmpDTMFToSend <> -1 Then
                                    Write_Log("", $"Process_DTMFOnConnected(DTMF) nLine: {CallInformation.nLine}, Index: {index + 1}, Playing: {tmpDTMFToSend}")
                                    ovsControl.Call_SendDTMF(0, CallInformation.nLine, tmpDTMFToSend, tmpDTMFMethod, tmpDTMFDuration, tmpDTMFPlay)
                                    Write_Log("", $"Process_DTMFOnConnected(DTMF) nLine: {CallInformation.nLine}, Index: {index + 1}, Played: {tmpDTMFToSend}")
                                Else
                                    Write_Log("", $"Process_DTMFOnConnected(DTMF) nLine: {CallInformation.nLine}, Index: {index + 1}, Played: {tmpDTMFToSend}, Invalid")
                                End If
                            Catch ex As Exception
                                Write_Log("", $"Process_DTMFOnConnected(DTMF) nLine: {CallInformation.nLine}, Index: {index + 1}, Played: {tmpDTMFToSend}, Exception:{ex.Message}")
                            End Try
                            Thread.Sleep(200)
                        Next
                    Else
                        Write_Log("ERR", "Process_DTMFOnConnected() - DTMF Not Found, Check Configuration")
                    End If
                Catch ex As Exception
                    Write_Log("ERR", "Process_DTMFOnConnected() - While sending DTMF:Error=" & ex.Message)
                End Try
            Else
                Write_Log("", "Process_DTMFOnConnected() Disabled To send DTMF")
            End If
        Catch ex As Exception
            Write_Log("ERR", "Process_DTMFOnConnected(Exception)" & ex.Message)
        End Try
        Process_DTMFOnConnected = True
    End Function
            ...
            Select Case e.ReminderText
                ...
                Case "StartRecordingWithDeley"
                    Try
                        If CallInformation.CallStatus = "Talking" Then
                            '' call the asterisk API if needed
                            Dim updateThread As New Thread(Sub() UpdateINFOForPABX("Connected"))
                            updateThread.Start()
                ...
                Case "ConnectDB"
                    ...
                    Catch exx As Exception
                        SetupScreen("Login", "")
                        Write_Log("ERR", " Connection Failed with Exception:" & exx.Message & ", InnerException:" & exx.InnerException.Message)
                        updateStatusText("Database connecting failed(" & exx.Message & ")")
                        MessageBox.Show("Error connecting database. " & vbNewLine & "Exception:" & exx.Message & ", InnerException:" & exx.InnerException.Message)
                        Exit Sub
                    End Try
    ...
    Private Sub DoDefaultConfiguration()
        ...
        Dim tmpString As String = "" & GetINIString("Main", "RecordCalls", "1")
        Select Case tmpString
            Case 1  '' Internal Recording
                Write_Log("", "DoDefaultConfiguration : Internal Recording")
                Configurations.RecordCalls = True
                Configurations.RecordingFormat = "wav"
                If LCase(GetINIString("Main", "RecordingFormat", "wav")) = "mp3" Then
                    Configurations.RecordingFormat = "mp3"
                End If
            Case 2      ' ovsPBX is recording this call
                Write_Log("", "DoDefaultConfiguration : PABX Recording")
                Configurations.RecordCalls = False
            Case Else   ' and 0 dont do anything about call recording
                Write_Log("", "DoDefaultConfiguration : PABX Recording RecordCalls=" & tmpString)
                Configurations.RecordCalls = False
        End Select
        ...
        Configurations.ExtensionRenaming = False
        Configurations.IgnoreRenameError = False

        If GetINIString("Renaming", "ExtensionRenamingX", "0") = "1" Then
            Configurations.ExtensionRenaming = True
            If GetINIString("Renaming", "IgnoreRenameError", "0") = "1" Then
                Configurations.IgnoreRenameError = True
            End If
        Else
            If GetINIStringOldWay("Renaming", "ExtensionRenaming", "0") = "1" Then
                Configurations.ExtensionRenaming = True
                If GetINIString("Renaming", "IgnoreRenameError", "0") = "1" Then
                    Configurations.IgnoreRenameError = True
                End If
        ...
        Try
            ...
            Select Case xScreen
                Case "Ready"
                    ...
                    CallInformation.Monitoring = False
    ...
    Private Sub Button_Login_Click(sender As Object, e As EventArgs) Handles Button_Login.Click
        ...
        Try
            ...
            If wsClientAllowed = True Then
                ...
                End If
            End If
            Write_Log("", "Button_Login(Default)" & "RowCount:" & "Starting verification " & ", " & TextBox_LoginName.Text & ", bIngnorePassword=" & bIngnorePassword)
            ...
            Write_Log("", "Button_Login(Default)" & "RowCount:" & rDataTable.Rows.Count & ", " & TextBox_LoginName.Text & ", bIngnorePassword=" & bIngnorePassword)
            ...
            If rDataTable.Rows.Count <= 0 Then
                ...
                Exit Sub
            End If
            Write_Log("", "Button_Login(Default)" & "Credentials verfyied for " & TextBox_LoginName.Text & ", bIngnorePassword=" & bIngnorePassword)
            ...
            If AgentInformation.LastName = "PULSE" Then
                'AgentInformation.LastName = ""
            Else
    ...
    Private Sub Process_CallDisconnected(Caller As String, Callee As String, sessionID As String, SIPString As String)
        Try
            ...
            If SIPString = "Rejected incoming call due to DND" Then
                Write_Log("", "Process_CallDisconnected(End) nLine:" & CallInformation.nLine & "," & CallInformation.CallStatus & " is ignored as its a disconnect msg for rejected call")
                Exit Sub
            End If
            If SIPString = "e.data2=486,Rejected incoming call due to DND" Then
                Write_Log("", "Process_CallDisconnected(End) nLine:" & CallInformation.nLine & "," & CallInformation.CallStatus & " is ignored as its a disconnect msg for rejected call")
                Exit Sub
            End If
    ...
    Private Sub Process_LoggedInSuccess(ByVal sLocalURI As String)
        Try
            ...
            End If
            ' HERE Required to Rename the Extension 
            sendTowsClinet("{" & """cmd"":{""ts"":""" & tsString & """,""Login"":" & """Rename""" & ",""State"":""" & "start" & """,""loginsessionid"":""" & AgentInformation.LoginSessionID & """ }" & "}")
            If Configurations.ExtensionRenaming = True Then
                updateStatusText("Renaming Enabled")
                Application.DoEvents()
                'Configurations.IgnoreRenameError = False
                Dim APIClient As New APIConnector.Connector(LogDir, 1, "", "", "", 0, "sgBOK")
                Try
                    Dim apiSERVER As String = GetINIString("Renaming", "apiSERVER", "")
                    Dim apiUser As String = GetINIString("Renaming", "apiUser", "")
                    Dim apiPassword As String = GetINIString("Renaming", "apiPassword", "")
                    Dim apiTimeOut As Integer = 5
                    Dim sTmpString As String
                    sTmpString = Trim(GetINIString("Renaming", "apiTimeout", "10"))
                    If IsNumeric(sTmpString) = True Then
                        apiTimeOut = Val(sTmpString)
                    End If

                    Dim tmpAgentName As String = ""
                    If AgentInformation.LastName = "PULSE" Then
                        tmpAgentName = AgentInformation.FirstName
                    Else
                        tmpAgentName = AgentInformation.FirstName & "-" & AgentInformation.LastName
                    End If

                    Write_Log("", "LoggedInSuccess() ExtensionRenaming=" & "True, apiSERVER=" & apiSERVER & ", apiUser=" & apiUser & ", apiPassowrd=" & "" & ", Extension=" & AgentInformation.PABXExt & ", agentName=" & tmpAgentName & ", apiPassowrd=" & "" & ", apiTimeOut=" & apiTimeOut)
                    updateStatusText("Connecting IP-PBX")
                    Application.DoEvents()
                    If APIClient.API_AVAYA_SET_UserName(apiSERVER, apiUser, apiPassword, AgentInformation.PABXExt, "", tmpAgentName, apiTimeOut) Then
                        Write_Log("", "LoggedInSuccess() ExtensionRenamed=" & "True, apiSERVER=" & apiSERVER & ", apiUser=" & apiUser & ", apiPassowrd=" & "" & ", apiPassowrd=" & "" & ", Extension=" & AgentInformation.PABXExt & ", agentName=" & AgentInformation.LoggedIn & "(" & AgentInformation.FirstName & "-" & AgentInformation.LastName & ")" & ", apiTimeOut=" & apiTimeOut)
                        sendTowsClinet("{" & """cmd"":{""ts"":""" & tsString & """,""Login"":" & """Rename""" & ",""State"":""" & "done" & """,""Exception"":""" & "" & """ }" & "}")
                        SetupScreen("DND", "NR-Login")
                    End If
                Catch ex As Exception
                    Write_Log("", "LoggedInSuccess() ExtensionRenamed=" & "False, Exception=" & ex.Message)
                    sendTowsClinet("{" & """cmd"":{""ts"":""" & tsString & """,""Login"":" & """Rename""" & ",""State"":""" & "failed" & """,""Exception"":""" & ex.Message & """ }" & "}")
                    If Configurations.IgnoreRenameError = True Then
                        SetupScreen("DND", "NR-Login")
                    Else
                        updateStatusText("Rename API Failed, cant continue, see the logs")
                        Call Process_Logout()
                    End If
                End Try
            Else
            ...
            End If
        Catch ex As Exception
            Write_Log("", "Process_LoggedInSuccess() Exception:" & ex.Message)
            MessageBox.Show(Me, "Process_LoggedInSuccess() Exception:" & ex.Message)
        End Try
    End Sub
    ...
    Private Sub Process_Logout()
        Try
            ...
            If Configurations.ExtensionRenaming = True Then
                Write_Log("", "Process_Logout() nLine:" & CallInformation.nLine & ", ExtensionRenaming=" & "Enabled")
                updateStatusText("Renaming Enabled")
                Application.DoEvents()
                'Configurations.IgnoreRenameError = False
                Try
                    ...
                    If IsNumeric(sTmpString) = True Then
                        apiTimeOut = Val(sTmpString)
                    End If
                    Dim tmpAgentName As String = "Ext " & AgentInformation.PABXExt
                    Write_Log("", "Process_Logout(rename) ExtensionRenaming=" & "True, apiSERVER=" & apiSERVER & ", apiUser=" & apiUser & ", apiPassowrd=" & "" & ", Extension=" & AgentInformation.PABXExt & ", agentName=" & tmpAgentName & ", apiPassowrd=" & "" & ", apiTimeOut=" & apiTimeOut)
                    updateStatusText("Connecting IP-PBX")
                    Application.DoEvents()
                    If APIClient.API_AVAYA_SET_UserName(apiSERVER, apiUser, apiPassword, AgentInformation.PABXExt, "", tmpAgentName, apiTimeOut) Then
                        Write_Log("", "Process_Logout() ExtensionRenamed=" & "True, apiSERVER=" & apiSERVER & ", apiUser=" & apiUser & ", apiPassowrd=" & "" & ", apiPassowrd=" & "" & ", Extension=" & AgentInformation.PABXExt & ", agentName=" & AgentInformation.LoggedIn & "(" & AgentInformation.FirstName & "-" & AgentInformation.LastName & ")" & ", apiTimeOut=" & apiTimeOut)
                    End If
                Catch ex As Exception
                    Write_Log("", "Process_Logout(Error) nLine:" & CallInformation.nLine & " rename " & ex.Message)
                End Try
            End If

            If Configurations.ExtPooling = True Then
                Write_Log("", "Process_Logout() ExtPooling:" & "Enabled")
                Try
                    Call dbClass.Extenstion_Release_FromPool(Val(AgentInformation.AgentID), Val(AgentInformation.PoolID))
                    Write_Log("", "Process_Logout(Extenstion_Release_FromPool) nLine:" & CallInformation.nLine & ", Extenstion Released (" & AgentInformation.PABXExt & ")")
                Catch ex As Exception
                    Write_Log("", "Process_Logout(Extenstion_Release_FromPool) nLine:" & CallInformation.nLine & ", Extenstion Released (" & AgentInformation.PABXExt & ") Exception:" & ex.Message)
                End Try
            End If
            ...
            If GetINIString("Main", "wsClientAllowed", "0") = "1" Then
                wsClientAllowed = True
                '' Changes made on Aug/05/2025
                '' Closing the applicaiton upon logon from PULSE
                If GetINIString("Main", "AutoCloseOnLogout", "1") = "1" Then
                    Write_Log("", "Process_Logout(End) nLine:" & "found Main, AutoCloseOnLogout=1 so time to close the app")
                    Application.DoEvents()
                    End
                End If
                'Call SetupwsClient()
    ...
    Private Sub Process_CallRinging(ChannelNo As Long, LineNo As Long, Caller As String, Callee As String, sessionID As String, SIPString As String)
        ...
        Try
            Write_Log("", "******************************************************************************************************************************************")
            Write_Log("", "Process_CallRinging(Start) nLine:" & CallInformation.nLine & ", Transfering = " & CallInformation.Transfering & ", IVR = " & IVRisWorking & ", Monitoring = " & CallInformation.Monitoring)
...
processyoualive:
                    'msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""logout"":""Not logged-in"",""loginsessionid"":""" & AgentInformation.LoginSessionID & """ }" & "}"
                    'msgFromWSClinet = $"{{""cmd"":{{""ts"":""{tsString}"",""LoggedIn"":""{AgentInformation.LoggedIn}"",""loginsessionid"":""{AgentInformation.LoginSessionID}""},""AgentID"":""{AgentInformation.AgentID}""}}}"
...
processGetAgentLoginMsg:
                        ...
                        Else
                            ...
                            Dim tmpAgentIDNew As String = tmpAgentID
                            If tmpAgentID.Contains(",") Then
                                Dim xxxx As String() = tmpAgentID.Split(",")
                                tmpAgentID = xxxx(0)
                                AgentInformation.FirstName = Trim(xxxx(1))
                                AgentInformation.LastName = "PULSE"
                                Write_Log("", "LoginInfo= " & AgentInformation.FirstName & ", Received:" & "From PULSE")
                            Else
                                Write_Log("", "LoginInfo= " & AgentInformation.FirstName & ", Not Received:")
                            End If
                            Try
                                TextBox_LoginName.Text = Trim(tmpAgentID)
                            Catch ex As Exception
                            End Try
                            Try
...
DoDisconnectSorry:
                    ...
                    ElseIf InStr(1, TheCommandText, "cmd{ccpingenatm,") > 0 Then
                        '' requirs InteractionID
                        ''         SessionID
                        ''         Language
                        ''         CNIC
                        ...
                        If CallInformation.CallStatus <> "Talking" Then
                            Write_Log("", "ccpingenatm Start CallStatus= " & CallInformation.CallStatus & ", IVRisWorking:" & IVRisWorking & " Not Talking")
                            msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""pingenatm"":" & """Sorry""" & ",""loginsessionid"":""" & AgentInformation.LoginSessionID & """ }" & "}"
                            sendTowsClinet(msgFromWSClinet)
                        Else
                            If IVRisWorking Then
                                Write_Log("", "ccpingenatm Start CallStatus= " & CallInformation.CallStatus & ", IVRisWorking:" & IVRisWorking & " IVR Already working")
                                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""pingenatm"":" & """Sorry""" & ",""loginsessionid"":""" & AgentInformation.LoginSessionID & """ }" & "}"
                                sendTowsClinet(msgFromWSClinet)
                            Else
                                Write_Log("", "ccpingenatm Start CallStatus= " & CallInformation.CallStatus & ", IVRisWorking:" & IVRisWorking & " IVR Actually starting")
                                IVR_ccATMPINGen(TheCommandText)
                                Write_Log("", "ccpingenatm Start CallStatus= " & CallInformation.CallStatus & ", IVRisWorking:" & IVRisWorking & " IVR End")
                                IVRisWorking = False
                            End If
                        End If
                    ...
                    ElseIf InStr(1, TheCommandText, "cmd{pingenatm,") > 0 Then
                        ...
                        ' Changes made on may/22/2025 for live call monitoring
                    ElseIf InStr(1, TheCommandText, "cmd{monitor,") > 0 Then
                        If AgentInformation.LoggedIn = False Then
                            msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""monitor"":" & """Not logged-in""" & ","",""loginsessionid"":""" & AgentInformation.LoginSessionID & """ }" & "}"
                            sendTowsClinet(msgFromWSClinet)
                            Exit Sub
                        End If
                        If CallInformation.CallDirection <> "Outbound" Then
                            msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""monitor"":" & """Not in outbound mode""" & ","",""loginsessionid"":""" & AgentInformation.LoginSessionID & """ }" & "}"
                            sendTowsClinet(msgFromWSClinet)
                            Exit Sub
                        End If
                        If Label_AgentStatus.Text <> "State:Idle" Then
                            msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""monitor"":" & """State is not idle""" & ","",""loginsessionid"":""" & AgentInformation.LoginSessionID & """ }" & "}"
                            sendTowsClinet(msgFromWSClinet)
                            Exit Sub
                        End If

                        Dim monitorInfo As String = ""
                        monitorInfo = Mid(TheCommandText, 13)
                        monitorInfo = Replace(monitorInfo, "}", "")

                        'On Error Resume Next
                        If Trim(monitorInfo) = "" Then
                            msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""monitor"":" & """monitorInfo not provided""" & ","",""loginsessionid"":""" & AgentInformation.LoginSessionID & """ }" & "}"
                            sendTowsClinet(msgFromWSClinet)
                            Exit Sub
                        End If
                        Dim monitorInfoX() As String
                        monitorInfoX = Split(monitorInfo, ",")
                        If monitorInfoX.Length < 2 Then
                            If Trim(monitorInfo) = "" Then
                                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""monitor"":" & """not enough information provided""" & ","",""loginsessionid"":""" & AgentInformation.LoginSessionID & """ }" & "}"
                                sendTowsClinet(msgFromWSClinet)
                                Exit Sub
                            End If
                        End If
                        Dim agentIDMonitorMode As String = "sniff"
                        If monitorInfoX.Count >= 1 Then
                            Select Case LCase(Trim(monitorInfoX(0)))
                                'Case "monitor"
                                Case "coach"
                                    agentIDMonitorMode = "coach"
                                Case "conf"
                                    agentIDMonitorMode = "conf"
                                Case Else
                                    agentIDMonitorMode = "sniff"
                            End Select
                        End If
                        Dim monitorString As String = GetINIString("Monitoring", agentIDMonitorMode & "-code", "*89*{EXT}").ToString.ToUpper
                        Dim agentIDForMonitoring As String = Trim(monitorInfoX(1))
                        Dim agDT As DataTable
                        agDT = dbClass.Get_Agent(agentIDForMonitoring)
                        If agDT.Rows.Count <= 0 Then
                            msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""monitor"":" & """Sorry, Agent not available""" & ",""loginsessionid"":""" & AgentInformation.LoginSessionID & """ }" & "}"
                            sendTowsClinet(msgFromWSClinet)
                            Exit Sub
                        End If
                        If agDT(0)("Loggedin") = "0" Then
                            msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""monitor"":" & """Sorry, Agent not loggedin""" & ",""loginsessionid"":""" & AgentInformation.LoginSessionID & """ }" & "}"
                            sendTowsClinet(msgFromWSClinet)
                            Exit Sub
                        End If
                        'If agDT(0)("State") = "Idle" Then
                        'msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""monitor"":" & """Sorry, Agent is idle""" & ",""loginsessionid"":""" & AgentInformation.LoginSessionID & """ }" & "}"
                        'sendTowsClinet(msgFromWSClinet)
                        'Exit Sub
                        'End If
                        '' Here now i have the Extenstion which needs to monitored
                        agentIDForMonitoring = "" & agDT(0)("PABXExt")
                        Write_Log("", "Ext For Monitoring= " & agentIDForMonitoring & ",monitorString=" & monitorString)
                        Try
                            monitorString = monitorString.Replace("{EXT}", agentIDForMonitoring)
                            monitorString = monitorString.Replace("{ext}", agentIDForMonitoring)
                            'monitorString = Replace(monitorString, "{EXT}", agentIDForMonitoring)
                            Write_Log("", "Ext For Monitoring= " & agentIDForMonitoring & ",monitorString now =" & monitorString)
                            TextBox_Number.Text = monitorString
                        Catch ex As Exception
                        End Try
                        CallInformation.Monitoring = True
                        Button_Call_Connect_Click(sender:=Me, e:=New EventArgs)
                        msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""monitor"":" & """Starting""" & ",""loginsessionid"":""" & AgentInformation.LoginSessionID & """ }" & "}"
                        sendTowsClinet(msgFromWSClinet)
                    ElseIf InStr(1, TheCommandText, "cmd{callout,") > 0 Then

                        '' Do Process Callout
                        '' 0 = requirs Nmber to call on
                        '' 1 = InteractionID
                        '' 2 = SessionID
                        ...
                        Else
                            ...
                            If UBound(sNewCMDBroken) < 3 Then
                                updateStatusText("Error: Packet Missing")
                                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""callout"":" & """something is missing""" & ","",""loginsessionid"":""" & AgentInformation.LoginSessionID & """ }" & "}"
                                sendTowsClinet(msgFromWSClinet)
                                Exit Sub
                            End If


                            If Trim(sNewCMDBroken(1)) = "" Then
                                updateStatusText("Error: number to call missing or invalid")
                                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""callout"":" & """Number to call missing or invalid""" & ","",""loginsessionid"":""" & AgentInformation.LoginSessionID & """ }" & "}"
                                sendTowsClinet(msgFromWSClinet)
                                Exit Sub
                            End If
                            tmpNumberToCall = sNewCMDBroken(1)

                            '' replace : from the number to call with ;
                            '' as we cant sent , in actual msg

                            tmpNumberToCall = Replace(tmpNumberToCall, ";", ",")
                            If GetINIStringOldWay("Outbound", "CheckBlankInteractionID", "1") = "1" Then
                                If Trim(sNewCMDBroken(2)) = "" Then
                                    updateStatusText("Error: InteractionID missing or invalid")
                                    msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""callout"":" & """InteractionID missing or invalid""" & ","",""loginsessionid"":""" & AgentInformation.LoginSessionID & """ }" & "}"
                                    sendTowsClinet(msgFromWSClinet)
                                    Exit Sub
                                End If
                                CallInformation.CRMInteractionID = sNewCMDBroken(2)
                            End If
                            '' changes on  oct-2024
                            If GetINIStringOldWay("Outbound", "CheckBlankSessionID", "1") = "1" Then
                                If Trim(sNewCMDBroken(3)) = "" Then
                                    updateStatusText("Error: SessionID missing or invalid")
                                    msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""callout"":" & """SessionID missing or invalid""" & ","",""loginsessionid"":""" & AgentInformation.LoginSessionID & """ }" & "}"
                                    sendTowsClinet(msgFromWSClinet)
                                    Exit Sub
                                End If
                                CallInformation.CRMSessionID = sNewCMDBroken(3)
                            End If
                            Try
...
#Region "IVRFunctions"
    ...
    Private Sub IVR_ccATMPINGen(cmd As String)
        Dim msgFromWSClinet As String
        ...
        Try
            ...
            Write_Log("", "IVR_ccATMPINGen:Start:" & cmd)
            ...
            Dim tsString As String = ""
            msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""ccpingenatm"":""Start"",""infoValidation"":""" & "Start" & """ }" & "}"
            sendTowsClinet(msgFromWSClinet)

            If UBound(sNewCMDBroken) < 4 Then
                updateStatusText("Error: Packet Missing")
                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""ccpingenatm"":""Start"",""infoValidation"":""" & "Packet Missing" & """ }" & "}"
                sendTowsClinet(msgFromWSClinet)
                Exit Sub
            End If
            ...
            For i = 0 To UBound(sNewCMDBroken) - 1
                If Len(Trim(sNewCMDBroken(i))) = 0 Then
                    ...
                    msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""ccpingenatm"":""Start"",""infoValidation"":""" & "Empty or invalid packet" & """ }" & "}"
                    sendTowsClinet(msgFromWSClinet)
                    Exit Sub
                End If
            Next
            ...
            If IsNumeric(sNewCMDBroken(4)) = False Then
                ...
                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""ccpingenatm"":""Start"",""infoValidation"":""" & "CNIC Invalid " & sNewCMDBroken(4) & """ }" & "}"
                sendTowsClinet(msgFromWSClinet)
                Exit Sub
            End If
            ...
            If Len(sCardNo) <> 16 Then
                ...
                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""ccpingenatm"":""Start"",""infoValidation"":""" & "Card Invalid " & sNewCMDBroken(5) & """ }" & "}"
                sendTowsClinet(msgFromWSClinet)
                Exit Sub
            End If
...
GetATMPIN1Again:
            Write_Log("", "ccATMPIN entering in taking ccATMPIN iCurrentTryCount=[" & iCurrentTryCount & "]")

            iCurrentTryCount = iCurrentTryCount + 1
            Application.DoEvents()
            updateStatusText("Playing ccATMPIN1 (ct=" & iCurrentTryCount & ")")
            ...
            sInputs = ""
            If Not _TerminalServerSession Then
                IVR_PlayFileAndGetDigits("\Sounds\" & sLanguage & "\ccATMPINNNewEnter.wav", "ATM PIN 1", 21, 4, "*#0123456789")
            End If
            Application.DoEvents()

            If CallInformation.CallStatus <> "Talking" Then
                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""ccpingenatm"":""Start"",""infoValidation"":""" & "Not talking" & """ }" & "}"
                sendTowsClinet(msgFromWSClinet)
                Exit Sub
            End If
            If Len(sInputs) = 0 Then
                ...
                If Not _TerminalServerSession Then
                    ...
                    Application.DoEvents()
                    IVR_PlayFileAndGetDigits("\Sounds\" & sLanguage & "\ccATMPINNoInput.wav", "No Input", 10, 4, "A")
                End If
                Application.DoEvents()

                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""ccpingenatm"":""Start"",""currenttry"":""" & iCurrentTryCount & """,""infoValidation"":""" & "NoInput" & """ }" & "}"
                sendTowsClinet(msgFromWSClinet)
            ...
            If (Len(sInputs) <> 4) Or (IsNumeric(sInputs) = False) Then
                ...
                If Not _TerminalServerSession Then
                    ...
                    IVR_PlayFileAndGetDigits("\Sounds\" & sLanguage & "\ccATMPINInvalidLength.wav", "Invalid Lenght", 10, 4, "A")
                End If

                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""ccpingenatm"":""Start"",""currenttry"":""" & iCurrentTryCount & """,""infoValidation"":""" & "Invalidlength" & """ }" & "}"
                sendTowsClinet(msgFromWSClinet)
                If iCurrentTryCount < iMaxInputTries Then
                    Application.DoEvents()
                    If Not _TerminalServerSession Then
...
MaxTries:
                ...
                If Not _TerminalServerSession Then
                    ...
                    IVR_PlayFileAndGetDigits("\Sounds\" & sLanguage & "\ccATMPINMaxTries.wav", "Max tries done", 10, 4, "A")
                End If
                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""ccpingenatm"":""Start"",""currenttry"":""" & iCurrentTryCount & """,""infoValidation"":""" & "Max Attempts Done" & """ }" & "}"
                sendTowsClinet(msgFromWSClinet)
                Exit Sub
            End If
...
GetTPIN2Again:
            ...
            sInputs = ""
            updateStatusText("ccATMPIN 1, Collecting, ccATMPIN 2, TRY=" & iCurrentTryCountPIN2)
            sInputs = ""
            msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""ccpingenatm"":""Start"",""infoValidation"":""" & "New ATMPIN Confirmation" & """ }" & "}"
            sendTowsClinet(msgFromWSClinet)
            ...
            If Not _TerminalServerSession Then
                ...
                IVR_PlayFileAndGetDigits("\Sounds\" & sLanguage & "\ccATMPINreEnter.wav", "ATM PIN 2", 21, 4, "*#0123456789")
            End If
            Application.DoEvents()
            ...
            If Len(sInputs) = 0 Then
                ...
                If Not _TerminalServerSession Then
                    ...
                    IVR_PlayFileAndGetDigits("\Sounds\" & sLanguage & "\ccATMPINNoInput.wav", "No Input", 10, 4, "A")
                End If
                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""ccpingenatm"":""Start"",""currenttry"":""" & iCurrentTryCountPIN2 & """,""infoValidation"":""" & "NoInput" & """ }" & "}"
                sendTowsClinet(msgFromWSClinet)
            ...
            If (Len(sInputs) <> 4) Or (IsNumeric(sInputs) = False) Then
                ...
                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""ccpingenatm"":""Start"",""currenttry"":""" & iCurrentTryCountPIN2 & """,""infoValidation"":""" & "Invalidlength" & """ }" & "}"
                sendTowsClinet(msgFromWSClinet)
                If iCurrentTryCountPIN2 < iMaxInputTries Then
                    Application.DoEvents()
                    If Not _TerminalServerSession Then
            ...
            If (sInputs <> TPIN1) Then
                updateStatusText("ccATMPIN ccATMPIN2 iMaxInputTries [" & iMaxInputTries & "] iCurrentTryCount [" & iCurrentTryCount & "]")
                If iCurrentTryCount < iMaxInputTries Then
                    Application.DoEvents()
                    If Not _TerminalServerSession Then
                        updateStatusText("Playing not same [" & iMaxInputTries & "] iCurrentTryCount [" & iCurrentTryCount & "]")
                        IVR_PlayFileAndGetDigits("\Sounds\" & sLanguage & "\ccATMPINNotSame.wav", "PIN not same", 10, 4, "A")
                    End If
                    Application.DoEvents()
                    If Not _TerminalServerSession Then
                        ...
                        'ovsControl.media_play(New ovsPSProPhony.TControl.media_play_Args(0, 0, CallInformation.nLineSessionID1, False, "\Sounds\" & sLanguage & "\tryAgain.wav", True, "A"))
                    End If
                    msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""ccpingenatm"":""Start"",""currenttry"":""" & iCurrentTryCount & """,""infoValidation"":""" & "PIN Not Same" & """ }" & "}"
                    sendTowsClinet(msgFromWSClinet)
                    GoTo GetATMPIN1Again
                Else
                    Application.DoEvents()
            ...
            Write_Log("", "ccATMPIN ccATMPIN2 also is good now, Calling ATMPIN Maker API")
            updateStatusText("Both PINs are good")

            msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""ccpingenatm"":""Start"",""infoValidation"":""" & "Calling API" & """ }" & "}"
            sendTowsClinet(msgFromWSClinet)
            ...
            updateStatusText("Calling API")
            MakeATMPINResponse = MakeATMPIN(sInputs, sCardNo, True)
            ...
            If MakeATMPINResponse = True Then
                ...
                Application.DoEvents()
                If Not _TerminalServerSession Then
                    IVR_PlayFileAndGetDigits("\Sounds\" & sLanguage & "\ccATMPINDone.wav", "ATM PIN Done", 10, 4, "A")
                End If
                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""ccpingenatm"":""Start"",""infoValidation"":""" & "Done" & """ }" & "}"
                sendTowsClinet(msgFromWSClinet)
                updateStatusText("API Response=" & MakeATMPINResponse)
                Write_Log("", "IVR_ccATMPINGen:ResponseSent:True:" & "" & " (" & msgFromWSClinet & ")")
            Else
                updateStatusText("API Response=" & MakeATMPINResponse & " - " & sTempResponseString)
                Application.DoEvents()
                If Not _TerminalServerSession Then
                    IVR_PlayFileAndGetDigits("\Sounds\" & sLanguage & "\ccATMPINFailed.wav", "ATM PIN Failed", 10, 4, "A")
                End If
                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""ccpingenatm"":""Start"",""infoValidation"":""" & "Failed" & """ ,""InfoDescription"":""" & sTempResponseString & """ }" & "}"
                '& """ ,""ResponseString"":""" & sTempResponseString & """
                sendTowsClinet(msgFromWSClinet)
                Write_Log("", "IVR_ccATMPINGen:ResponseSent:False:" & "" & " (" & msgFromWSClinet & ")")
                updateStatusText("API Response=" & MakeATMPINResponse & " - " & sTempResponseString)
            End If
        Catch ex As Exception

            msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""ccpingenatm"":""Start"",""infoValidation"":""" & "Failed" & """ ,""InfoDescription"":""" & "Exception:" & ex.Message & """ }" & "}"
            sendTowsClinet(msgFromWSClinet)
            Write_Log("", "IVR_ccATMPINGen:Error:" & "" & " (" & ex.Message & ")")
            updateStatusText("IVR_ccATMPINGen:Error:" & "" & " (" & ex.Message & ")")
        End Try
...
GetTPIN2Again:
    ...
    '    Private Function UpdateINFOForPABX(EventName As String) As Boolean
    '        UpdateINFOForPABX = False

    '        Dim xAPIMaxTries As Long = 2
    '        Dim xAPICurrentTries As Long = 0

    '        Try
    '            xAPIMaxTries = 2
    '            xAPIMaxTries = Val(GetINIString("ovsAPI", "APIRetries", "2"))
    '        Catch ex As Exception
    '            xAPIMaxTries = 2
    '        End Try
    ...
    '        Dim ovsAPIURL As String = ""
    '        Dim ovsAPIUser As String = ""
    '        Dim ovsAPIPassword As String = ""

    '        ovsAPIURL = GetINIString("ovsAPI", "APIUpdatePABXInfo", "")
    '        ovsAPIUser = GetINIString("ovsAPI", "APIUpdatePABXInfoUser", "admin")
    '        ovsAPIPassword = GetINIString("ovsAPI", "APIUpdatePABXInfoPassword", "password")
    '        Dim sTmpString As String
    '        sTmpString = Trim(GetINIString("ovsAPI", "APIUpdatePABXInfoTimeout", "5"))
    '        If IsNumeric(sTmpString) = True Then
    '            xTimeOut = Val(sTmpString)
    '        End If
    '        Write_Log("", "UpdateINFOForPABX:Not Nothing,xTimeOut=" & xTimeOut & ",ovsAPIURL=" & ovsAPIURL)
    '        Dim tmpText As String = ""
    ...
    '        Try
    '            Dim newUUID As String = Guid.NewGuid().ToString()
    '            Dim currentTimestamp As String = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")
    '            Dim jsonString As String = $"{{" &
    '                                      $"""Event"": ""{EventName}""," &
    '                                      $"""PABXExt"": ""{AgentInformation.PABXExt}""," &
    '                                      $"""AgentName"": ""{AgentInformation.FirstName} {AgentInformation.LastName}""," &
    '                                      $"""AgentID"": ""{AgentInformation.LoginName}""," &
    '                                      $"""Direction"": ""{CallInformation.CallDirection}""," &
    '                                      $"""CallerID"": ""{CallInformation.CalledID}""," &
    '                                      $"""DB_CallID"": ""{CallInformation.RefID_DB}""," &
    '                                      $"""CRM_InteractionID"": ""{CallInformation.CRMInteractionID}""," &
    '                                      $"""CRM_SessionID"": ""{CallInformation.CRMSessionID}""," &
    '                                      $"""Other1"": ""{CallInformation.CallerID}""," &
    '                                      $"""Other2"": ""{TextBox_Number.Text}""," &
    '                                      $"""Other3"": ""Data3""," &
    '                                      $"""timestamp_sent"": ""{currentTimestamp}""," &
    '                                      $"""timestamp_responded"": ""{currentTimestamp}""," &
    '                                      $"""timestamp_received"": ""{currentTimestamp}""," &
    '                                      $"""requestID"": ""{newUUID}""" &
    '                                      $"}}"

    '            Try
    '                If Trim(ovsAPIURL) <> "" Then
    '                    Write_Log("", "UpdateINFOForPABX:URL=" & ovsAPIURL & ", TimeOut=" & xTimeOut & ", jsonString=" & jsonString)
    '                    updateStatusText("Now calling API Timeout(" & xTimeOut & ")")
    '                    xAPICurrentTries = xAPICurrentTries + 1
    '                    tmpText = APIClient.PABX_UpdateInfo(ovsAPIURL, "admin", "password", jsonString, xTimeOut, responseCode, responseMSG)
    '                    sTempResponseString = "" & tmpText & ""
    '                    Write_Log("", "UpdateINFOForPABX:Response:" & "" & " (" & sTempResponseString & "), responseCode=" & responseCode & ", responseMSG=" & responseMSG)
    '                End If
    '                Return UpdateINFOForPABX
    '            Catch ex As Exception
    '                Write_Log("", "UpdateINFOForPABX:ExceptionX:" & "" & " (" & ex.Message & "," & sTempResponseString & "), responseCode=" & responseCode & ", responseMSG=" & responseMSG)
    '            End Try
    '        Catch ex As Exception
    '            Write_Log("", "UpdateINFOForPABX:ExceptionY:" & "" & " (" & ex.Message & "," & sTempResponseString & "), responseCode=" & responseCode & ", responseMSG=" & responseMSG)
    '        End Try
    '        Try
    '            Write_Log("", "UpdateINFOForPABX:After xAPICurrentTries:" & "" & " (" & xAPICurrentTries & ", xAPIMaxTries" & xAPIMaxTries & ")")
    '            If xAPICurrentTries < xAPIMaxTries Then
    '                Write_Log("", "UpdateINFOForPABX:Give Another try xAPICurrentTries:" & xAPICurrentTries & ")")
    '                GoTo tryAgain
    '            ElseIf xAPICurrentTries > xAPIMaxTries Then
    '                Write_Log("", "UpdateINFOForPABX:Max Tries done xAPICurrentTries:" & xAPICurrentTries & "), Now Disconnecting")
    '                Button_Call_Hangup_Click(Me, e:=New EventArgs)
    '            End If
    '        Catch ex As Exception
    '            Write_Log("", "UpdateINFOForPABX:ExceptionZ:" & "" & " (" & ex.Message & "," & sTempResponseString & "), responseCode=" & responseCode & ", responseMSG=" & responseMSG)
    '        End Try
    '        Return UpdateINFOForPABX
    '    End Function
    Private Function UpdateINFOForPABX(EventName As String) As Boolean
        Dim xAPIMaxTries As Long = 2
        Dim xAPIDelayBetweenTries As Long = 2
        Dim xDTMFOnConnectedPostDelay As Long = 1

        If CallInformation.Monitoring = True Then
            Write_Log("", $"UpdateINFOForPABX - Ignore the Calling API: {CallInformation.Monitoring}")
            Return True
        End If
        If CallInformation.CallStatus <> "Talking" Then
            Write_Log("", $"UpdateINFOForPABX - Before Calling API: CallStatus: NOT TALKING, IGNORE API Call")
            Return True ' Success
        End If
        ...
        End Try
        Try
            xAPIDelayBetweenTries = Val(GetINIString("ovsAPI", "APIDelayBetweenRetries", "2"))
        Catch ex As Exception
        End Try
        Try
            xDTMFOnConnectedPostDelay = Val(GetINIString("ovsAPI", "DTMFOnConnectedPostDelay", "1"))
        Catch ex As Exception
        End Try
        Dim xAPICurrentTries As Long = 0
        Dim responseCode As Integer = -1
        ...
        Dim ovsAPIURL As String = GetINIString("ovsAPI", "APIUpdatePABXInfo", "")
        If Trim(ovsAPIURL) = "" Then
            Write_Log("", $"UpdateINFOForPABX - Before Calling API: {ovsAPIURL}, Calling is disabled")
            Return False
        End If
        Call Process_DTMFOnConnected()
        If xDTMFOnConnectedPostDelay > 0 Then
            Threading.Thread.Sleep(xDTMFOnConnectedPostDelay * 1000)
        End If
        Dim ovsAPIUser As String = GetINIString("ovsAPI", "APIUpdatePABXInfoUser", "admin")
        Dim ovsAPIPassword As String = GetINIString("ovsAPI", "APIUpdatePABXInfoPassword", "password")
        Dim xTimeOut As Long = 5
        Try
            xTimeOut = Val(GetINIString("ovsAPI", "APIUpdatePABXInfoTimeout", "5"))
        Catch ex As Exception
        End Try
        xAPICurrentTries = 0
        Do
            Try
                Write_Log("", $"UpdateINFOForPABX - Before Calling API: {ovsAPIURL}, APICurrentTries: {xAPICurrentTries}, CallStatus:{CallInformation.CallStatus}, APIDelayBetweenTries:{xAPIDelayBetweenTries}")
                xAPICurrentTries += 1
                Dim APIClient As New APIConnector.Connector(LogDir, 1, "", "", "", 0, "sgBOK")
                Dim jsonString As String = $"{{" &
                                        $"""Event"": ""{EventName}""," &
                                        ...
                                        $"""APICurrentTry"": ""{xAPICurrentTries}""," &
                                        $"""APIMaxTries"": ""{xAPIMaxTries}""," &
                                        $"""APIDelayBetweenTries"": ""{xAPIDelayBetweenTries}""," &
                                        $"""requestID"": ""{Guid.NewGuid()}""" &
                                        $"}}"

                Write_Log("", $"UpdateINFOForPABX - After Making API: {ovsAPIURL}, Payload: {jsonString}, APICurrentTries: {xAPICurrentTries}, CallStatus:{CallInformation.CallStatus}, APIDelayBetweenTries:{xAPIDelayBetweenTries}")
                Dim tmpText As String = APIClient.PABX_UpdateInfo(ovsAPIURL, ovsAPIUser, ovsAPIPassword, jsonString, xTimeOut, responseCode, responseMSG)
                Write_Log("", $"UpdateINFOForPABX - API Response: Code={responseCode}, Message={responseMSG}, Data={tmpText}, CallStatus:{CallInformation.CallStatus}, APIDelayBetweenTries:{xAPIDelayBetweenTries}")
                If responseCode = 200 Then
                    Return True ' Success
                End If
            Catch ex As Exception
                Write_Log("", $"UpdateINFOForPABX - Exception during API call:Message:{ex.Message}, InnerException:{ex.InnerException}, xAPICurrentTries: {xAPICurrentTries}, CallStatus:{CallInformation.CallStatus}, APIDelayBetweenTries:{xAPIDelayBetweenTries}")
            End Try

            If xAPICurrentTries = xAPIMaxTries Then
                Write_Log("", "UpdateINFOForPABX - Max retries reached, stopping API attempts.")
                Exit Do
            End If
            Write_Log("", "UpdateINFOForPABX - Retrying API call, xAPICurrentTries=" & xAPICurrentTries & ", APIDelayBetweenTries=" & xAPIDelayBetweenTries & " sec(s)")
            If xAPIDelayBetweenTries > 0 Then
                Threading.Thread.Sleep(xAPIDelayBetweenTries * 1000) ' Optional: Delay between retries
            End If
        Loop
        Try
            Write_Log("", "UpdateINFOForPABX - Dropping Call")
            Button_Call_Hangup_Click(Me, e:=New EventArgs)
            Write_Log("", "UpdateINFOForPABX - Call Dropped")
        Catch ex As Exception
            Write_Log("", "UpdateINFOForPABX - Exception while dropping call")
        End Try
        Return False ' Failure
    End Function

    Private Function MakeATMPIN(ATMPIN As String, CardNo As String, Optional CreditCard As Boolean = False) As Boolean
        MakeATMPIN = False
        sTempResponseString = "" & "" & ""
        Try
            MakeATMPIN = False
            ...
            Dim sTmpString As String
            If CreditCard Then
                sTmpString = Trim(GetINIString("sgAPI", "APICreateccATMPINTimout", "5"))
            Else
            ...
            Dim sgAPIURL As String
            If CreditCard Then
                sgAPIURL = GetINIString("sgAPI", "APICreateccATMPIN", "http://18.188.178.46:8100" & "/api/CustomerPINValidation/pin/createTelPin")
            Else
                sgAPIURL = GetINIString("sgAPI", "APICreateATMPIN", "http://18.188.178.46:8100" & "/api/CustomerPINValidation/pin/createTelPin")
            End If
            Dim sgAPITOKEN As String
            sgAPITOKEN = GetINIString("sgAPI", "APITOKEN", "TOKEN")
            Write_Log("", "MakeATMPIN:Not Nothing,xTimeOut=" & xTimeOut & ",sgAPIURL=" & sgAPIURL & " CreditCard=" & CreditCard)
            Dim tmpText As String = ""

            Write_Log("", "MakeATMPIN:URL=" & sgAPIURL & ", TimeOut=" & xTimeOut & ", sgAPITOKEN= CreditCard=" & CreditCard)
            'On Error Resume Next
            ...
            If tmpText <> "200" Then
                ...
                updateStatusText("ATMPIN not created CreditCard=" & CreditCard)
            Else
                ...
                updateStatusText("ATMPIN Created CreditCard=" & CreditCard)
                MakeATMPIN = True
            End If

        Catch ex As Exception
            Write_Log("", "MakeATMPIN:Error:" & "" & " (" & ex.Message & ") CreditCard=" & CreditCard)
        End Try
    End Function
