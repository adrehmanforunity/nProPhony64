...
Public Class Form_Splash
    ...
    Private Sub IVR_PlayFileAndGetDigits(szFileNames As String, Label As String, TimeOut As Integer, Digits As Integer, Optional TermDigits As String = "*#1234567890", Optional AllowCutThrough As Boolean = False)
        sInputs = ""

        If Trim(TermDigits) = "" Then
            TermDigits = "*#0123456789"
        End If
        TermDigits = UCase(TermDigits)
        ...
        Try
            ...
            If TimeOut = -1 Then
                Exit Sub
            End If
            Dim bPlayingStatus As Boolean = False
            Do While True

                If UCase(Trim(TermDigits)) = "A" Then
                    updateStatusText(Label & ":Digits(0/0), " & xDateDiff & ", tm=" & DateTime.Now.ToString("ss:ffff").ToString())
                Else
                    updateStatusText(Label & ":P(" & bPlayingStatus.ToString.PadLeft(5, CChar(" ")) & ") Digits(" & (Digits - Len(sInputs) & "/" & Digits) & "), " & xDateDiff & ", tm=" & DateTime.Now.ToString("ss:ffff").ToString())
                End If
                bPlayingStatus = ovsControl.Playing
                Application.DoEvents()
                If AllowCutThrough = False Then
                    If ovsControl.Playing Then
                        sInputs = ""
                    End If
                End If
                Application.DoEvents()
                ...
                If IVRisWorking = False Then
                    Exit Sub
                End If
                'If TermDigits <> "A" And sInputs <> "" Then
                'End If
                If sInputs <> "" Then
                    If TermDigits <> "A" Then
                        If TermDigits.Contains(Strings.Right(sInputs, 1)) Then
                            Exit Do
                        End If
                    End If
                End If
...
#End Region
    ...
    Private Sub Form_Splash_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        AgentInformation.AppVersion = "Efforts-31.3"
            ...
            Select Case e.ReminderText
                ...
                Case "wsInitialize"
                    ...
                    Try
                        ...
                        End If
                    Catch ex As Exception
                        Write_Log("", "Event_Occured() wsRestarted Exception:" & ex.Message)
                        MessageBox.Show(Me, "Please ensure there is no other session is running of ProPhony Application", "Exception", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    End Try
        ...
        Try
            ...
            Select Case CallInformation.CallStatus
                ...
                Case Else
                    ...
                    updateStatusText("Call Closed Received" & CallInformation.CallerID)
                    'RunUploader()
                    Try
        ...
        Try
            Select Case e.eventName
                ...
                Case "onInviteClosed", "onInviteFailure"
                    Process_CallDisconnected("", "", "" & e.sessionId, e.data1)
    ...
    Private Sub Button_Call_Hangup_Click(sender As Object, e As EventArgs) Handles Button_Call_Hangup.Click
        Try
            Write_Log("", "Call_Hangup_Click(Start) nLine:" & CallInformation.nLine)
            ...
            If CallInformation.CallDirection = "Inbound" Then
                If CallInformation.CallStatus = "Ringing" Then
                    Write_Log("", "Connect_Click(In) Here to Reject a call")
                    ovsControl.Call_Reject(0, CallInformation.nLine)
                Else
                    Write_Log("", "Connect_Click(In) Here to Hangup a call")
                    ovsControl.Call_Hangup(0, CallInformation.nLine)
                End If
            Else
                Write_Log("", "Connect_Click(Out) Here to Hangup a call")
                If CallInformation.CallStatus = "Ringing" Then
...
#Region "IVRFunctions"
    ...
    Private Sub IVR_TPINVerify(cmd As String)
        Try
            ...
            'End If
            sInputs = ""
            IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\TPINExisting.wav", "TPIN Existing", 21, 4, "A", False)
            ...
            If Len(sInputs) = 0 Then
                ...
                'End If
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\TPINNoInput.wav", "No Inputs", 0, 1, "A", False)
            ...
            If (Len(sInputs) <> 4) Or (IsNumeric(sInputs) = False) Then
                ...
                'End If
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\TPINInvalidLength.wav", "No Inputs", 0, 1, "A", False)
...
#Region "New Code on Aug-18-2023"
            '' here need to check, if want to say thanks to the customer, and ask for wait a bit till API is being called.
            If GetINIString("IVRProPhony", "announcementPreAPI", "1") = "0" Then
                Write_Log("", "Announcement PreAPI, is disabled")
            Else
                If CallInformation.CallStatus <> "Talking" Then
                    Write_Log("", "After Information collection Call Disconnected!")
                    updateStatusText("Call Disconnected!")
                    Exit Sub
                End If
                updateStatusText("Playing Thankyou and wait")
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\announcementPreAPI.wav", "Thanks and Wait", 0, 1, "A", False)
            End If
            '' check if required to play clock ticking sound, while waiting for API response
            If GetINIString("IVRProPhony", "clockTicking", "1") = "0" Then
                Write_Log("", "Clock Ticking, is disabled")
            Else
                If CallInformation.CallStatus <> "Talking" Then
                    Write_Log("", "Call Disconnected!")
                    updateStatusText("Call Disconnected!")
                    Exit Sub
                End If
                updateStatusText("Ticking Played")
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\clockTicking.wav", "Wait Sound", -1, 0, "A", False)
            End If

            If CallInformation.CallStatus <> "Talking" Then
                updateStatusText("Caller Disconnected, before API Call")
                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""pingentpin"":""Start"",""infoValidation"":""" & "Not Calling API - Caller Disconnected" & """ }" & "}"
                sendTowsClinet(msgFromWSClinet)
                Exit Sub
            End If
            If IVRisWorking = False Then
            ...
            End If
#End Region
            updateStatusText("Calling API,Please wait")
            '' call the API
            ...
            If MakeTPINResponse = True Then
                ...
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\TPINVerifyed.wav", "TPIN Verifyed", 0, 1, "A", False)
            ...
            Else
                updateStatusText("Calling API Response: " & MakeTPINResponse)
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\TPINInvalid.wav", "TPIN Invalid", 0, 1, "A", False)
...
GetATMPIN1Again:
            ...
            If Not _TerminalServerSession Then
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\ATMPINNNewEnter.wav", "ATM PIN 1", 21, 4, "A")
            End If
            ...
            If Len(sInputs) = 0 Then
                ...
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\ATMPINNoInput.wav", "No Input", 10, 0, "A", False)
                ...
                If iCurrentTryCount < iMaxInputTries Then
                    ...
                    IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\tryagain.wav", "Try Again", 0, 1, "A", False)
            ...
            If (Len(sInputs) <> 4) Or (IsNumeric(sInputs) = False) Then
                ...
                If Not _TerminalServerSession Then
                    ...
                    IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\ATMPINInvalidLength.wav", "Invalid Lenght", 0, 1, "A", False)
                End If
...
MaxTries:
                ...
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\ATMPINMaxTries.wav", "Max tries done", 0, 1, "A", False)
...
GetTPIN2Again:
            ...
            IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\ATMPINreEnter.wav", "ATM PIN 2", 15, 4, "*#", False)
            Application.DoEvents()
            ...
            If Len(sInputs) = 0 Then
                ...
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\ATMPINNoInput.wav", "No Input", 0, 1, "A", False)
            ...
            If (sInputs <> TPIN1) Then
                ...
                If iCurrentTryCount < iMaxInputTries Then
                    ...
                    If Not _TerminalServerSession Then
                        ...
                        IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\ATMPINNotSame.wav", "PIN not same", 0, 1, "A", False)
                    End If
                    Application.DoEvents()
...
#End Region
            ...
            If MakeATMPINResponse = True Then
                ...
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\ATMPINDone.wav", "ATM PIN Done", 0, 1, "A", False)
            ...
            Else
                ...
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\ATMPINFailed.wav", "ATM PIN Failed", 0, 1, "A", False)
    ...
    Private Sub IVR_TPINGen_NEW(cmd As String)
        Dim msgFromWSClinet As String
        ...
        Try
            ...
            Dim iCurrentTryCountPIN2 As Integer = 0
            IVRisWorking = True
            Write_Log("", "IVR_TPINGen_NEW:Start:" & cmd)
            ...
            For i = 0 To UBound(sNewCMDBroken) - 1
                If Len(Trim(sNewCMDBroken(i))) = 0 Then
                    updateStatusText("Error: Empty or Invalid packet")
                    msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""pingentpin"":""Start"",""infoValidation"":""" & "Empty or invalid packet" & """ }" & "}"
                    sendTowsClinet(msgFromWSClinet)
                    Exit Sub
                End If
            Next
            ...
            'If Len(sCardNo) <> 16 Then
            'updateStatusText("Error: Card Invalid")
            'msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""pingentpin"":""Start"",""infoValidation"":""" & "Card Invalid " & sNewCMDBroken(5) & """ }" & "}"
            'sendTowsClinet(msgFromWSClinet)
            'Exit Sub
            'End If
...
GetTPIN1Again:
            ...
            iCurrentTryCount = iCurrentTryCount + 1
            Application.DoEvents()
            updateStatusText("Playing TPIN1 (ct=" & iCurrentTryCount & ")")

            sInputs = ""
            IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\TPINNNewEnter.wav", "TPIN 1", 21, 4, "A")
            Application.DoEvents()
            ...
            If Len(sInputs) = 0 Then
                ...
                updateStatusText("Playing NoInput (ct=" & iCurrentTryCount & ")")
                Application.DoEvents()
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\TPINNoInput.wav", "No Input", 0, 1, "A", False)
                Application.DoEvents()
            ...
            If (Len(sInputs) <> 4) Or (IsNumeric(sInputs) = False) Then
                Application.DoEvents()
                updateStatusText("Playing Invalid Lenght (ct=" & iCurrentTryCount & ")")
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\TPINInvalidLength.wav", "Invalid Lenght", 0, 1, "A", False)
...
MaxTries:
                Application.DoEvents()
                updateStatusText("Maximum tries done")
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\TPINMaxTries.wav", "Max tries done", 0, 1, "A", False)
...
GetTPIN2Again:
            ...
            IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\TPINreEnter.wav", "TPIN 2", 21, 4, "A")
            Application.DoEvents()
            Debug.WriteLine(DateTime.Now().ToString & " return after 4 Digits Y " & sInputs)
            ...
            If (sInputs <> TPIN1) Then
                ...
                If iCurrentTryCount < iMaxInputTries Then
                    Application.DoEvents()
                    updateStatusText("Playing not same [" & iMaxInputTries & "] iCurrentTryCount [" & iCurrentTryCount & "]")
                    IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\TPINNotSame.wav", "PIN not same", 0, 1, "A", False)
                    Application.DoEvents()
                    updateStatusText("Playing Try Again [" & iMaxInputTries & "] iCurrentTryCount [" & iCurrentTryCount & "]")
                    IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\tryAgain.wav", "Try Again", 0, 1, "A", False)
...
#End Region
            ...
            If MakeTPINResponse = True Then
                ...
                CallInformation.CallerVerifyed = 2
                Application.DoEvents()
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\TPINDone.wav", "TPIN PIN Done", 0, 1, "A", False)
                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""pingentpin"":""Start"",""infoValidation"":""" & "Done" & """ }" & "}"
                sendTowsClinet(msgFromWSClinet)
                updateStatusText("API Response=" & MakeTPINResponse)
                Write_Log("", "IVR_TPINGen_NEW:ResponseSent:True:" & "" & " (" & msgFromWSClinet & ")")
            Else
                updateStatusText("API Response=" & MakeTPINResponse & " - " & sTempResponseString)
                Application.DoEvents()
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\TPINFailed.wav", "TPIN PIN Failed", 0, 1, "A", False)
                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""pingentpin"":""Start"",""infoValidation"":""" & "Failed" & """ ,""InfoDescription"":""" & sTempResponseString & """ }" & "}"
                sendTowsClinet(msgFromWSClinet)
                Write_Log("", "IVR_TPINGen_NEW:ResponseSent:False:" & "" & " (" & msgFromWSClinet & ")")
                updateStatusText("API Response=" & MakeTPINResponse & " - " & sTempResponseString)
            End If
        Catch ex As Exception
            msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""pingentpin"":""Start"",""infoValidation"":""" & "Failed" & """ ,""InfoDescription"":""" & "Exception:" & ex.Message & """ }" & "}"
            sendTowsClinet(msgFromWSClinet)
            Write_Log("", "IVR_TPINGen_NEW:Error:" & "" & " (" & ex.Message & ")")
            updateStatusText("IVR_TPINGen_NEW:Error:" & "" & " (" & ex.Message & ")")
        End Try
...
GetTPIN1Again:
            ...
            If Len(TPIN1) = 0 Then
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\TPINNoInput.wav", "No Input", 0, 1, "A")
                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""pingentpin"":""Start"",""currenttry"":""" & iCurrentTryCount & """,""infoValidation"":""" & "NoInput" & """ }" & "}"
                sendTowsClinet(msgFromWSClinet)
                If iCurrentTryCount < iMaxInputTries Then
                    Application.DoEvents()
                ...
                End If
            End If
            If (Len(TPIN1) <> 4) Or (IsNumeric(TPIN1) = False) Then
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\TPINInvalidLength.wav", "Invalid", 0, 1, "A")
                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""pingentpin"":""Start"",""currenttry"":""" & iCurrentTryCount & """,""infoValidation"":""" & "Invalidlength" & """ }" & "}"
                sendTowsClinet(msgFromWSClinet)
                If iCurrentTryCount < iMaxInputTries Then
                    IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\tryagain.wav", "Try Again", 0, 1, "A")
                    GoTo GetTPIN1Again
                Else
                    GoTo MaxTries
                End If
...
MaxTries:
                Application.DoEvents()
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\TPINMaxTries.wav", "Max Tries", 0, 1, "A")
                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""pingentpin"":""Start"",""currenttry"":""" & iCurrentTryCount & """,""infoValidation"":""" & "Max Attempts Done" & """ }" & "}"
                sendTowsClinet(msgFromWSClinet)
                Exit Sub
            End If
...
GetTPIN2Again:
            ...
            If CallInformation.CallStatus <> "Talking" Then
                ...
                Exit Sub
            End If
            IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\TPINreEnter.wav", "TPIN Confirm", 21, 4, "A")
            If IVRisWorking = False Then
                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""pingentpin"":""Start"",""infoValidation"":""" & "Not talking" & """ }" & "}"
                sendTowsClinet(msgFromWSClinet)
                Exit Sub
            ...
            If Len(TPIN2) = 0 Then
                ...
                End If
            End If
            If (Len(TPIN2) <> 4) Or (IsNumeric(TPIN2) = False) Then
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\TPINInvalidLength.wav", "Invalid", 0, 1, "A")
                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""pingentpin"":""Start"",""currenttry"":""" & iCurrentTryCountPIN2 & """,""infoValidation"":""" & "Invalidlength" & """ }" & "}"
                sendTowsClinet(msgFromWSClinet)
                If iCurrentTryCountPIN2 < iMaxInputTries Then
            ...
            If (TPIN2 <> TPIN1) Then
                updateStatusText("TPIN TPIN2 iMaxInputTries [" & iMaxInputTries & "] iCurrentTryCount [" & iCurrentTryCount & "]")
                If iCurrentTryCount < iMaxInputTries Then
                    IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\TPINNotSame.wav", "Not Same", 0, 1, "A")
                    Application.DoEvents()
                    IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\tryAgain.wav", "Try Again", 0, 1, "A")
                    msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""pingentpin"":""Start"",""currenttry"":""" & iCurrentTryCount & """,""infoValidation"":""" & "PIN Not Same" & """ }" & "}"
                    sendTowsClinet(msgFromWSClinet)
...
#End Region
            ...
            If MakeTPINResponse = True Then
                updateStatusText("API Response=" & MakeTPINResponse)
                CallInformation.CallerVerifyed = 2
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\TPINDone.wav", "TPIN Done", 0, 1, "A")
                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""pingentpin"":""Start"",""infoValidation"":""" & "Done" & """ }" & "}"
                sendTowsClinet(msgFromWSClinet)
                Write_Log("", "IVR_TPINGen:ResponseSent:True:" & "" & " (" & msgFromWSClinet & ")")
            Else
                IVR_PlayFileAndGetDigits("Sounds\" & sLanguage & "\TPINFailed.wav", "TPIN Failed", 0, 1, "A")
                updateStatusText("API Response=" & MakeTPINResponse)
                msgFromWSClinet = "{" & """cmd"":{""ts"":""" & tsString & """,""pingentpin"":""Start"",""infoValidation"":""" & "Failed" & """ ,""InfoDescription"":""" & sTempResponseString & """ }" & "}"
                sendTowsClinet(msgFromWSClinet)
        ...
        Catch ex As Exception
            ...
            'MessageBox.Show(Me, "IVR_TPINGen:" & "" & " (" & ex.Message & ")")
        End Try

    End Sub

    Private Sub IVR_TPINGen_last(cmd As String)
        Try
...
#Region "New Code on Aug-18-2023"
            ...
            Else
                ...
                If Not _TerminalServerSession Then
                    updateStatusText("Playing Thankyou and wait")
                    ovsControl.media_play(New ovsPSProPhony.TControl.media_play_Args(0, 0, CallInformation.nLineSessionID1, False, "Sounds\" & sLanguage & "\announcementPreAPI.wav", True, "A"))
                End If
            End If
            '' check if required to play clock ticking sound, while waiting for API response
            If GetINIString("IVRProPhony", "clockTicking", "1") = "0" Then
            ...
            Else
                ...
                If Not _TerminalServerSession Then
                    updateStatusText("Ticking Played")
                    ovsControl.media_play(New ovsPSProPhony.TControl.media_play_Args(0, 0, CallInformation.nLineSessionID1, True, "Sounds\" & sLanguage & "\clockTicking.wav", True, "A"))
                End If
            End If
            If CallInformation.CallStatus <> "Talking" Then
                updateStatusText("Caller Disconnected, before API Call")
