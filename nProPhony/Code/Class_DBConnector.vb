...
Public Class Class_DBConnector
    Private WSDBConnecotor As WSDBConnector
    ...
    Public Function SetCallInformation(Data1 As String, Data2 As String) As Boolean
        ...
        Try
            ...
            Select Case Data1
                Case "CallEnd"
                    querytext = querytext & ""
                    If _myDatabaseType = "mssql" Or _myDatabaseType = "wsdb" Then
                ...
                Case "ACW"
                    Select Case _myDatabaseType
                        ...
                        Case "mssql", "wsdb" : querytext = "update TBL_AgentCalls set TBL_AgentCalls.CallDurationACW=DATEDIFF(second,      TBL_AgentCalls.DateTimeHangup , GETDATE()) where TBL_AgentCalls.RowID=" & CallInformation.RefID_DB
                ...
                Case "NR-Login"
                    ...
                    If dtResult.Rows.Count <= 0 Then
                        Select Case _myDatabaseType
                            ...
                            Case "mssql", "wsdb" : sQueryText = "INSERT INTO TBL_RealTime  VALUES('" & AgentInformation.PABXExt & "',-1," & AgentInformation.AgentID & ",'','','',1,GETDATE())"
                    ...
                    Select Case _myDatabaseType
                        ...
                        Case "mssql", "wsdb" : sQueryText = "Update TBL_RealTime set AgentID=" & AgentInformation.AgentID & ", Direction='login',State='Login',Data1='',Loggedin=1,DateTime_Updated=GETDATE() where PABXExt='" & AgentInformation.PABXExt & "'"
...
tryAgain:
        Try
            ...
            Select Case _myDatabaseType
                ...
                Case "wsdb"
                    'Dim resultTask As Task(Of DataTable) = WSDBConnecotor.RunQuery(Query, _myDatabaseQueryTimeout)
                    'Dim resultTable As DataTable = resultTask.GetAwaiter().GetResult()
                    Dim resultTable As DataTable = WSDBConnecotor.RunQuery(Query, _myDatabaseQueryTimeout)
                    Return resultTable
                Case Else
        ...
        Catch ex As Exception
            ...
            If tryCount = 0 Then
                ...
                Catch exx As Exception
                    Write_Log("", "RunDBQuery() Connection Failed with Exception:" & exx.Message & ", InnerException:" & exx.InnerException.Message & ", x=" & x & ", Query=" & Query)
                End Try
                'End If
            End If
    ...
    Public Function Activity_Get_SessionID(DNCaption As String, Optional InsertOnly As Boolean = False) As Boolean
        ...
        Try
            ...
            Select Case _myDatabaseType
                ...
                Case "mssql", "wsdb" : SQLString = "INSERT INTO TBL_AgentActivity VALUES(" & AgentInformation.ActivitySessionID & "," & AgentInformation.LoginSessionID & ",'" & DNCaption & "', GETDATE(),GETDATE(),0,''," & CallInformation.RefID_DB & ")"
    ...
    Public Function Activity_Set_SessionID(DNCaption As String) As Boolean
        ...
        Try
            ...
            Select Case _myDatabaseType
                ...
                Case "mssql", "wsdb" : SQLString = "update TBL_AgentActivity  set TBL_AgentActivity.ActivityEndDateTime = GETDATE(), TBL_AgentActivity.NewState ='" & DNCaption & "', TBL_AgentActivity.RefID_DB=" & CallInformation.RefID_DB & ", TBL_AgentActivity.ActivityDuration = DATEDIFF(second,      TBL_AgentActivity.ActivityStartDateTime, GETDATE()) where TBL_AgentActivity.RowID=" & AgentInformation.ActivitySessionID
    ...
    Public Function Login_Get_SessionID() As DataTable
        ...
        Try
            ...
            tmpAPPPath = AgentInformation.AppPath.Replace("\", "\\")

            Select Case _myDatabaseType
                Case "access" : TmpSQLString = "INSERT INTO TBL_AgentLoginSessions (RowID, AgentID, LoginDateTim, LogoutDateTime, FromComputerName, FromIPAddress, FromOSUser, AppTitle, App, AppDate, AppPath ) VALUES('" & AgentInformation.LoginSessionID & "'," & AgentInformation.AgentID & ", NOW(),NOW(),'" & AgentInformation.FromComputerName & "','" & AgentInformation.FromIPAddress & "','" & AgentInformation.FromOSUser & "','" & AgentInformation.AppTitle & "','" & AgentInformation.AppVersion & "','" & AgentInformation.AppDate & "', '" & tmpAPPPath & "')"
                Case "mssql", "wsdb" : TmpSQLString = "INSERT INTO TBL_AgentLoginSessions VALUES(" & AgentInformation.LoginSessionID & "," & AgentInformation.AgentID & ", GETDATE(),'','" & AgentInformation.FromComputerName & "','" & Mid(AgentInformation.FromIPAddress, 1, 20) & "','" & AgentInformation.FromOSUser & "','" & AgentInformation.AppTitle & "','" & AgentInformation.AppVersion & "','" & AgentInformation.AppDate & "', '" & tmpAPPPath & "')"
                Case Else : TmpSQLString = "INSERT INTO TBL_AgentLoginSessions VALUES(" & AgentInformation.LoginSessionID & "," & AgentInformation.AgentID & ", NOW(),NOW(),'" & AgentInformation.FromComputerName & "','" & Mid(AgentInformation.FromIPAddress, 1, 20) & "','" & AgentInformation.FromOSUser & "','" & AgentInformation.AppTitle & "','" & AgentInformation.AppVersion & "','" & AgentInformation.AppDate & "', '" & tmpAPPPath & "')"
            End Select
    ...
    Public Function CallRecording_Put() As String
        ...
        Try
            ...
            If GetINIString("Main", "ConversionToOPUS", "0") = "1" Then
                tmpFileName = CompressRecording(tmpFileName, "opus")
                CallInformation.RecordingFileName = tmpFileName
            ElseIf GetINIString("Main", "ConversionToOPUS1", "0") = "1" Then
                tmpFileName = CompressRecording(tmpFileName, "opus1")
                CallInformation.RecordingFileName = tmpFileName
            ElseIf GetINIString("Main", "ConversionToMP3", "0") = "1" Then
                tmpFileName = CompressRecording(tmpFileName, "mp3")
                CallInformation.RecordingFileName = tmpFileName
            ElseIf GetINIString("Main", "ConversionToMP31", "0") = "1" Then
                tmpFileName = CompressRecording(tmpFileName, "mp31")
                CallInformation.RecordingFileName = tmpFileName
            ElseIf GetINIString("Main", "ConversionToGSM", "0") = "1" Then
                tmpFileName = CompressRecording(tmpFileName, "gsm")
                CallInformation.RecordingFileName = tmpFileName
            End If
            ...
            Select Case _myDatabaseType
                ...
                Case "mssql", "wsdb"
                    quertText = ""
                    quertText = quertText & "INSERT INTO TBL_CallsData (RowID,RefID_DB,InsertMoment,CallData,InteractionID,SessionID)"
                    quertText = quertText & " VALUES('" & tmpRowID & "','" & CallInformation.RefID_DB & "','" & tmpDateTime & "','" & strBase64WavFile & "','" & CallInformation.CRMInteractionID & "','" & CallInformation.CRMSessionID & "')"
                Case Else
                    quertText = ""
                    quertText = quertText & "INSERT INTO TBL_CallsData (RowID,RefID_DB,InsertMoment,CallData)"
                    quertText = quertText & " VALUES('" & tmpRowID & "','" & CallInformation.RefID_DB & "','" & tmpDateTime & "','" & strBase64WavFile & "');"
            ...
            Select Case _myDatabaseType
                Case "wsdb"
                    Try
                        ...
                        WSDBConnecotor.SendData("Data", "" & CallInformation.RefID_DB, "0", CallInformation.CallerID, CallInformation.CallDirection, CallInformation.CRMInteractionID, CallInformation.CRMSessionID, AgentInformation.LoginName, System.IO.Path.GetFileName(tmpFileName), WavFile)
                    Catch ex As Exception
                        Write_Log("", "CallRecording_Put(" & RecordingTryCount & "):Error:" & ex.Message)
                        If RecordingTryCount = 0 Then
                    ...
                    End Try
                Case "access", "mssql"
                    sSQL = sSQL & "INSERT INTO TBL_CallsData(RowID,RefID_DB,InsertMoment,CallData,InteractionID,SessionID)"
                    sSQL = sSQL & "values (@tmpRowID,@RefID_DB,@tmpDateTime,@CallData,@InteractionID,@SessionID)"
                    ...
                    selectCMD.Parameters.AddWithValue("@InteractionID", CallInformation.CRMInteractionID)
                    selectCMD.Parameters.AddWithValue("@SessionID", CallInformation.CRMSessionID)
...
IgnoreThisOne:
    ...
    Public Function CloseDatabase(ByRef err) As Boolean
        ...
        Try
            Select Case _myDatabaseType
                ...
                Case "wsdb"
                    WSDBConnecotor = Nothing
                Case Else
                    MyODBCConnectionToMYSQL.Close()
            End Select
        Catch ex As Exception
    ...
    Public Function OpenDatabase(ByRef err) As Boolean
        ...
        Try
            ...
            Select Case _myDatabaseType
                ...
                Case "wsdb" : OpenDatabase = OpenWSDB(err)
    ...
    Public Sub New()
        Try
            ...
            Select Case LCase(GetINIStringOldWay("Database", "Type", "mysql"))
                ...
                Case "access"
                    ...
                    _myDatabasePort = "1443"
                Case "wsdb"
                    _myDatabaseType = "wsdb"
                    _myDatabasePort = "5000"
                Case Else
                    _myDatabaseType = "mssql"
                    _myDatabasePort = "1443"
            End Select

            If _myDatabaseType = "wsdb" Then
                Configurations.ResolveDoMainToIP = True
                If GetINIStringOldWay("Database", "ResolveDomainToIP", "1") = "0" Then
                    Configurations.ResolveDoMainToIP = False
                End If

                _myDatabaseIP = GetINIStringOldWay("Database", "DBComServer", "ws://192.168.100.8:5000?auth=AuthKey:your-auth-key")
                If Configurations.ResolveDoMainToIP Then
                    Write_Log("", "ResolveDomainToIP:is enabled before _myDatabaseIP:" & _myDatabaseIP)
                    Configurations.ResolveDoMainToIP = True
                    _myDatabaseIP = ResolveDomainToIPAsync(_myDatabaseIP)
                    Write_Log("", "ResolveDomainToIP:is enabled after _myDatabaseIP:" & _myDatabaseIP)
                End If
                _myDatabasePort = Int(GetINIStringOldWay("Database", "DatabasePort", "5000"))
            Else
            ...
            If _myDatabaseType = "wsdb" Then
                _myDatabaseConnectionTimeOut = _myDatabaseConnectionTimeOut * 1000
                _myDatabaseQueryTimeout = Int(GetINIStringOldWay("Database", "QueryTimeout", "5"))
                _myDatabaseQueryTimeout = _myDatabaseQueryTimeout * 1000
            End If
        ...
        End Try
    End Sub
    Private Function OpenWSDB(ByRef err As String) As Boolean
        OpenWSDB = False
        Try
            WSDBConnecotor = New WSDBConnector(_myDatabaseIP, Val(_myDatabaseConnectionTimeOut), 0)
            OpenWSDB = True
        Catch ex As Exception
            err = ex.Message
            'Throw New System.Exception(ex.Message)
            Throw New Exception("Exception OpenWSDB(): " & ex.Message, ex.InnerException)
        End Try
    End Function
    ...
    Public Function Server_Get_SessionID(ByRef Optional CallerID As String = "") As Boolean
        ...
        Try
            ...
            Write_Log("", "GetServerSessionID:Count:" & dtResult.Rows.Count & ",CallInformation.CustomerID:" & CallInformation.CustomerID & ",CRMSessionID: " & CallInformation.CRMSessionID & ", CRMInteractionID:" & CallInformation.CRMInteractionID)
            If dtResult.Rows.Count > 0 Then
    ...
    Private Sub UCallBack_Event_Occured(Sender As Object, e As Class_UCallBack.Event_Occured_Args) Handles UCallBack.Event_Occured
        ...
        End Try
    End Sub
    'Below Function Added By Atif 27 November 2023
    Public Sub InsertTransferToFedback(ByVal _Language As String)
        Dim tmpQueryString = ""
        Try
            Write_Log("", "InsertTransferToFedback Start : RowID:" & CallInformation.RefID_DB & " : Agent:" & AgentInformation.PABXExt & ": Verified:" & CallInformation.CallerVerifyed & "" & ": Language:" & _Language & "")
            Try
                Dim dtResult As New DataTable
                'tmpQueryString = "EXEC" & vbTab & "[SP_INSERT_CallsFromAgent] " & CallInformation.RefID_DB & ",'" & CallInformation.CallerID & "','" & "" & "','','','" & AgentInformation.PABXExt & "'," & CallInformation.CallerVerifyed & ",'" & _Language & "','" & CallInformation.CRMInteractionID & "','" & CallInformation.CRMSessionID & "';"
                tmpQueryString = "EXEC" & vbTab & "[SP_INSERT_CallsFromAgent] " _
                    & CallInformation.RefID_DB & "," _
                    & "'" & CallInformation.CallerID & "'," _
                    & "'" & CallInformation.CustomerID & "'," _
                    & "'" & CallInformation.CustomerID & "'," _
                    & "'" & AgentInformation.LoginName & "'," _
                    & "'" & Trim(Trim(AgentInformation.FirstName) & " " & Trim(AgentInformation.LastName)) & "'," _
                    & CallInformation.CallerVerifyed & "," _
                    & "'" & _Language & "'," _
                    & "'" & CallInformation.CRMInteractionID & "'," _
                    & "'" & CallInformation.CRMSessionID & "';"

                dtResult = RunDBQuery(tmpQueryString)
            Catch ex As Exception
                Throw New System.Exception(ex.Message & ", Query=" & tmpQueryString)
            End Try
            Write_Log("", "SP_INSERT_CTI_CallsFromAgent End:")
        Catch ex As Exception
            Write_Log("", "SP_INSERT_CTI_CallsFromAgent Error:" & ex.Message & ", Query=" & tmpQueryString)
    ...
    Protected Overrides Sub Finalize()
        ...
        MyBase.Finalize()
    End Sub
    Private Function ResolveDomainToIPAsync(domain As String) As String
        Try
            ' Parse the input domain into a Uri object
            Dim uri As Uri
            If Not Uri.TryCreate(domain, UriKind.Absolute, uri) Then
                ' If the input is not a valid URI, return the original input
                Return domain
            End If
            ...
            ' Declare the ipAddress variable
            Dim ipAddress As IPAddress

            ' Check if the hostname is already an IP address
            If ipAddress.TryParse(hostname, ipAddress) Then
                ' If it's already an IP address, return the original URI as a string
                Return domain
            End If
            ...
            If hostEntry.AddressList.Length = 0 Then
                ' If no IP addresses are found, return the original input
                Return domain
            End If
        ...
        Catch ex As Exception
            ' If an exception occurs, return the original input
            Return domain
        End Try
    End Function
End Class
