...
#End Region
    ...
    Private Sub Form_Splash_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        AgentInformation.AppVersion = "37.7+DTI-Disabled"
                                ...
                                Try
                                    ...
                                    If tmpDTMF <> "" Then
                                        ...
                                        For index = 0 To Len(tmpDTMF)
                                            ...
                                            Thread.Sleep(200)
                                        Next
                                        If tmpDTMFPlay = False Then
                                            GoTo PlayConnectedTone
                                        End If
                                    Else
                                        Write_Log("ERR", "DTMF Not Found, Check Configuration")
                                    End If
                                Catch ex As Exception
                                    Write_Log("ERR", "While sending DTMF:Error=" & ex.Message)
                                End Try
                            Else
PlayConnectedTone:
    ...
    Private Sub DoDefaultConfiguration()
        ...
        Select Case "" & GetINIString("Main", "RecordCalls", "1")
...
GetTPIN2Again:
    ...
    Private Function UpdateINFOForPABX(EventName As String) As Boolean
        ...
        Do
            Try
                Write_Log("", $"UpdateINFOForPABX - Before Calling API: {ovsAPIURL}, APICurrentTries: {xAPICurrentTries}")
                xAPICurrentTries += 1
                Dim APIClient As New APIConnector.Connector(LogDir, 1, "", "", "", 0, "sgBOK")
                Dim jsonString As String = $"{{" &
                                        $"""Event"": ""{EventName}""," &
                                        ...
                                        $"""CurrentTry"": ""{xAPICurrentTries}""," &
                                        $"""APIMaxTries"": ""{xAPIMaxTries}""," &
                                        $"""requestID"": ""{Guid.NewGuid()}""" &
                                        $"}}"

                Write_Log("", $"UpdateINFOForPABX - After Calling API: {ovsAPIURL}, Payload: {jsonString}, APICurrentTries: {xAPICurrentTries}")
                ...
                Write_Log("", $"UpdateINFOForPABX - API Response: Code={responseCode}, Message={responseMSG}, Data={tmpText}")
                If responseCode = 200 Then
                    Return True ' Success
                End If
            Catch ex As Exception
                Write_Log("", $"UpdateINFOForPABX - Exception during API call: {ex.Message}, xAPICurrentTries: {xAPICurrentTries}")
            End Try
            ...
            If xAPICurrentTries = xAPIMaxTries Then
                ...
                Exit Do
            End If
            Write_Log("", "UpdateINFOForPABX - Retrying API call, xAPICurrentTries=" & xAPICurrentTries)
            Threading.Thread.Sleep(1000) ' Optional: Delay between retries
        Loop
